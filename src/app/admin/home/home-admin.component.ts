...
export class HomeListComponent  {
   ...
   selectedClassId: number | null = null;
    ...
    onSubjectFilterChange() { this.applyFilters(); }
    onClassFilterChange() { this.applyFilters(); }

    applyFilters() {
      let list = this.allCheckins.slice();
      if (this.selectedSubjectId) {
        const subj = this.subjects.find(s=>s.id===this.selectedSubjectId)?.name;
        list = list.filter(ci => (ci.subject === this.selectedSubjectId) || (subj && ci.subject_name === subj));
      }
      if (this.selectedClassId) {
        list = list.filter(ci => (ci.user_face?.class_name === this.selectedClassId) || (ci.user_face?.class_name_name && this.classes.find(c=>c.id===this.selectedClassId)?.name === ci.user_face.class_name_name));
      }
  ...
  formatLateText(item: any): string {
    ...
    if (!item?.is_late || late <= 0) return 'Đúng giờ';
    if (late >= 60) {
